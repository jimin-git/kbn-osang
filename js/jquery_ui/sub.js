$(function(){
	...
	$('.btn_vow_popup').click(function(){
        $('.vow_popup').addClass('active');
        $('.popup_bg').addClass('on');                
    });
    
    $('.vow_popup .round_sub_btn button.gray').bind('click',function(){
		$('#PopUp > div').removeClass('active');
		$('.popup_bg').removeClass('on');
	});
	...
	$('.search_select_btn button').bind('click',function() {
		...
		if(state == 'selectBox_all'){
			//전체보기
			$('.thesis_year_list').addClass('all');
			$('.thesis_year_list > div').addClass('on');
		}else{
			$('.thesis_year_list').removeClass('all');
			//각각의 컨텐츠보여주기
	...
	//교육성과리스트
	$('.search_select_btn button.education_btn').bind('click',function() {
		var statetxt = jQuery('#thesis-selectBox option:selected').attr('name');
		$('.year_tit').text(statetxt);
	});
	...
	/*분양신청방법*/
	$('.saleapply_way_process ul li').bind('click mouseover',function(){
		$('.saleapply_way_process ul li').removeClass('active');
		$(this).addClass('active');

		$(this).parent().find('.saleapply_way_process_popup').mouseleave(function(){
			$('.saleapply_way_process_popup').parent().removeClass('active');
		});
	});
	...
	$('.kcdSearch_stap_wrap .kcdSearch_stap').each(function(index, item){
		$(item).addClass('stap_0' + index); //순서넣어주기
		$(this).find('ul li a').click(function(){
			if($(this).parents('.kcdSearch_stap').hasClass('stap_00')){
				$('.kcdSearch_stap ul li').removeClass('active');
				$(this).parent().addClass('active');
			}if($(this).parents('.kcdSearch_stap').hasClass('stap_01')){
				$('#Code4th li').removeClass('active');
				$(this).parent().parent().find('li').removeClass('active');
				$(this).parent().addClass('active');
			}if($(this).parents('.kcdSearch_stap').hasClass('stap_02')){
				$(this).parent().parent().find('li').removeClass('active');
				$(this).parent().addClass('active');
			}
		});
   ...
   var fileTarget = $('.filebox .upload-hidden');
	...
	//검색한글영어 선택구분
	$('.kcdSearch_lang_btn button').click(function(){
		$('.kcdSearch_lang_btn button').removeClass('on');
		$(this).addClass('on');
	});

	//분양현황 프로세스
	$('.my_sale_process_list > li').mouseover(function(){
		$('.my_sale_process_list > li').removeClass('on');
		$(this).addClass('on');
	})
	$('.my_sale_process_list > li div').mouseleave(function(){
		$('.my_sale_process_list > li').removeClass('on');
	});
